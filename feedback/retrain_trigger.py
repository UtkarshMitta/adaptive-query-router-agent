...
def run_retrain_pipeline(
    ...
    2. Merge feedback into training data
    ...
    print(f"\n📋 Step 1: Merging feedback into training data...")
    ...
    print(f"\n📋 Step 3: Triggering retraining...")
    try:
        trigger_retrain(method=method)
    except Exception as e:
...
def main():
    ...
    parser.add_argument("--merge-only", action="store_true", help="Only merge feedback, don't retrain")
    ...
    if args.check:
        ...
        if stats['total'] > 0:
            print(f"   Model accuracy on feedback: {stats['accuracy']:.1%}")
        return

    if args.merge_only:
        train_file = DATA_DIR / "train.jsonl"
        merge_feedback_into_dataset(collector.feedback_file, train_file)
        return

    if args.force:
        print("⚡ Force retrain requested")
        # Merge whatever feedback exists
        if collector.count > 0:
            train_file = DATA_DIR / "train.jsonl"
            merge_feedback_into_dataset(collector.feedback_file, train_file)

        trigger_retrain(method=args.method)
        collector.reset(archive=True)
        return
